...
"""Boilerplate-aligned runtime implementation for payload cache operations."""
...
from app.crud import fetch_payload_cache_rows, replace_payload_cache_index
from app.schemas.manifest import ManifestResponse
from app.services.capture.batch_lifecycle import upsert_analysis_batch
...
def get_payload_cache_summary() -> dict[str, Any]:
    ...
    try:
        cache_rows = fetch_payload_cache_rows()
    except Exception:
        cache_rows = []
...
def _update_cache_index(manifest: dict[str, Any]) -> None:
    ...
    replace_payload_cache_index(rows)
