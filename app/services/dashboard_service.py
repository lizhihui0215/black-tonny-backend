...
def _resolve_latest_analysis_batch_id(connection: Connection) -> str | None:
...
def _build_summary_from_serving(date_range: DateRangeSchema) -> dict[str, Any] | None:
    try:
        with get_serving_engine().begin() as connection:
            analysis_batch_id = _resolve_latest_analysis_batch_id(connection)
            ...
            compare_snapshot_rows = connection.execute(
                select(
                    ...
                    inventory_daily_snapshot.c.snapshot_date == date.fromisoformat(date_range.compareEndDate),
                )
            ).mappings().all()
