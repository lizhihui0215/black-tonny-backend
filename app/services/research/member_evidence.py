...
def build_member_http_evidence_chain(
    *,
    ...
    zero_match_values = [str(item["value"]) for item in search_variants if int(item["row_count"] or 0) == 0]
    exact_match_values = [str(item["value"]) for item in search_variants if int(item["row_count"] or 0) == 1]
    ...
    return {
        "member_center": {
            ...
            "baseline": baseline,
            "declared_total_count": int(baseline.get("row_count") or 0),
        ...
        "conclusion": {
            "member_center_mainline_ready": True,
