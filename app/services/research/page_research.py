...
class ResearchPageRegistryEntry:
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "scope_texts": list(build_page_scope_texts(self)),
...
def build_page_scope_texts(entry: ResearchPageRegistryEntry | Mapping[str, Any]) -> tuple[str, ...]:
    values: list[str] = []
    for key in ("title", "menu_target_title", "canonical_name", "variant_of"):
        value = getattr(entry, key, None) if not isinstance(entry, Mapping) else entry.get(key)
...
def build_page_scope_action_texts(entry: ResearchPageRegistryEntry | Mapping[str, Any]) -> tuple[str, ...]:
    recipe = getattr(entry, "recipe", None) if not isinstance(entry, Mapping) else entry.get("recipe")
    steps = getattr(recipe, "steps", ()) if recipe is not None else ()
    values: list[str] = []
    for step in steps:
        target_text = getattr(step, "target_text", None) if not isinstance(step, Mapping) else step.get("target_text")
...
def build_default_page_recipe(title: str) -> ResearchPageRecipe:
    ...
    elif canonical in {"收货确认", "门店盘点单"}:
        query_wait_ms = 5000

    steps = [
        ResearchStepRecipe(key="open", label="打开页面", kind="open", wait_ms=2500),
        ResearchStepRecipe(key="query", label="执行查询", kind="query", wait_ms=query_wait_ms),
    ]
    ...
    if canonical == "收货确认":
        steps.extend(
            [
                ResearchStepRecipe(
                    key="select_receipt_row",
                    label="选择首行单据",
                    kind="select_first_grid_row",
                    wait_ms=1200,
                ),
                ResearchStepRecipe(
                    key="click_receipt_confirm",
                    label="点击单据确认",
                    kind="click_text",
                    target_text="单据确认",
                    wait_ms=2200,
                ...
                ResearchStepRecipe(
                    key="select_receipt_row_for_logistics",
                    label="再次选择首行单据",
                    kind="select_first_grid_row",
                    wait_ms=1200,
                ),
                ResearchStepRecipe(
                    key="click_receipt_logistics",
                    label="点击物流信息",
                    kind="click_text",
                    target_text="物流信息",
                    wait_ms=2200,
    ...
    elif canonical == "门店盘点单":
        steps.extend(
            [
                ResearchStepRecipe(
                    key="select_stocktaking_row",
                    label="选择首行盘点单",
                    kind="select_first_grid_row",
                    wait_ms=1200,
                ),
                ResearchStepRecipe(
                    key="click_stocktaking_detail",
                    label="点击查看明细",
                    kind="click_text",
                    target_text="查看明细",
                    wait_ms=2200,
                ...
                ResearchStepRecipe(
                    key="click_stocktaking_barcode_record",
                    label="点击条码记录",
                    kind="click_text",
                    target_text="条码记录",
                    wait_ms=2200,
                ...
                ResearchStepRecipe(
                    key="click_stocktaking_profit_loss",
                    label="点击统计损溢",
                    kind="click_text",
                    target_text="统计损溢",
                    wait_ms=2200,
