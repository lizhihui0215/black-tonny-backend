...
CAPTURE_NAME_OVERRIDES: dict[str, tuple[str, bool, str]] = {
    ...
    "商品销售情况 / SelSaleReportData": ("product_sales_snapshot_records", True, "snapshot"),
    "客户资料 / SelDeptList": ("customer_master_records", True, "master"),
    "会员中心 / SelVipInfoList": ("member_profile_records", False, "raw"),
    "会员维护 / SelVipReturnVisitList": ("member_maintenance_records", True, "master"),
    "会员总和分析 / SelVipAnalysisReport": ("member_analysis_snapshot_records", True, "snapshot"),
    "会员消费排行 / SelVipSaleRank": ("member_sales_rank_snapshot_records", True, "snapshot"),
    "储值卡明细 / GetDIYReportData": ("stored_value_card_detail", True, "detail"),
    "储值卡汇总 / GetDIYReportData": ("stored_value_card_summary_snapshot_records", True, "snapshot"),
    "储值按店汇总 / GetDIYReportData": ("stored_value_by_store_snapshot_records", True, "snapshot"),
    "退货明细 / SelReturnStockList": ("return_document_lines", False, "raw"),
    "收货确认 / SelDocConfirmList": ("receipt_confirmation_documents", True, "document"),
    "门店盘点单 / SelDocManageList": ("store_stocktaking_documents", True, "document"),
    "门店盘点单 / store_stocktaking_diff_records": ("store_stocktaking_diff_records", True, "diff"),
    "每日流水单 / SelectRetailDocPaymentSlip": ("daily_payment_slips_snapshot", True, "snapshot"),
}
...
def build_capture_route_registry_from_board(board: dict[str, Any]) -> dict[str, Any]:
    ...
    blocked_mainline_candidates = [
        item["route"]
        for item in bindings
        if item["capture_role"] == "mainline_fact" and item["capture_status"] == "capture_candidate_blocked"
    ]
    ...
    summary = {
        ...
        "blocked_mainline_candidate_count": len(blocked_mainline_candidates),
        "blocked_mainline_candidates": blocked_mainline_candidates,
...
def render_capture_route_registry_markdown(registry: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- 仍阻塞的主链候选：`{summary['blocked_mainline_candidate_count']}`",
    ...
    if summary["blocked_mainline_candidates"]:
        lines.extend(["", "当前仍阻塞的主链候选："])
        for route in summary["blocked_mainline_candidates"]:
