...
MEMBER_SALES_RANK_SNAPSHOT_RECORDS_ENDPOINT = "member_sales_rank_snapshot_records"
MEMBER_SALES_RANK_SNAPSHOT_ROUTE_KIND = "snapshot"


def build_member_sales_rank_snapshot_capture_admission_bundle(
    *,
    member_sales_rank_snapshot_evidence: Mapping[str, Any],
    ...
    return {
        "member_sales_rank_snapshot": {
            "capture_route_name": MEMBER_SALES_RANK_SNAPSHOT_RECORDS_ENDPOINT,
            "capture_role": "snapshot",
            "route_kind": MEMBER_SALES_RANK_SNAPSHOT_ROUTE_KIND,
...
def persist_member_sales_rank_snapshot_capture_admission_bundle(
    *,
    ...
    snapshot_detail = dict(bundle["member_sales_rank_snapshot"])
    if not snapshot_detail["capture_admission_ready"]:
        raise ValueError("会员消费排行 snapshot capture 条件未满足: " + "；".join(snapshot_detail["blocking_issues"]))
    ...
    append_capture_payload(
        capture_batch_id,
        source_endpoint=MEMBER_SALES_RANK_SNAPSHOT_RECORDS_ENDPOINT,
        route_kind=MEMBER_SALES_RANK_SNAPSHOT_ROUTE_KIND,
        payload=baseline_payload,
        request_params={
            "route_kind": MEMBER_SALES_RANK_SNAPSHOT_ROUTE_KIND,
