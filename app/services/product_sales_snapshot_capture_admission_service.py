...
PRODUCT_SALES_SNAPSHOT_RECORDS_ENDPOINT = "product_sales_snapshot_records"
PRODUCT_SALES_SNAPSHOT_ROUTE_KIND = "snapshot"


def build_product_sales_snapshot_capture_admission_bundle(
    *,
    product_sales_snapshot_evidence: Mapping[str, Any],
    ...
    return {
        "product_sales_snapshot": {
            "capture_route_name": PRODUCT_SALES_SNAPSHOT_RECORDS_ENDPOINT,
            "capture_role": "snapshot",
            "route_kind": PRODUCT_SALES_SNAPSHOT_ROUTE_KIND,
...
def persist_product_sales_snapshot_capture_admission_bundle(
    *,
    ...
    snapshot_detail = dict(bundle["product_sales_snapshot"])
    if not snapshot_detail["capture_admission_ready"]:
        raise ValueError("商品销售情况 snapshot capture 条件未满足: " + "；".join(snapshot_detail["blocking_issues"]))
    ...
    append_capture_payload(
        capture_batch_id,
        source_endpoint=PRODUCT_SALES_SNAPSHOT_RECORDS_ENDPOINT,
        route_kind=PRODUCT_SALES_SNAPSHOT_ROUTE_KIND,
        payload=baseline_payload,
        request_params={
            "route_kind": PRODUCT_SALES_SNAPSHOT_ROUTE_KIND,
