...
from app.services.capture.batch_lifecycle import update_capture_batch, upsert_analysis_batch
from app.services.serving.summary_projection import (
...
def _load_capture_rows(capture_batch_id: str) -> tuple[dict[str, Any], dict[str, list[dict[str, Any]]]]:
    batch_row = fetch_capture_batch_row(capture_batch_id)
    if not batch_row:
        raise KeyError(f"capture batch not found: {capture_batch_id}")
    payload_rows = list_capture_endpoint_payload_rows(capture_batch_id)
...
def transform_capture_batch_to_serving(
    ...
    *,
    ...
    replace_summary_projection_rows(
        analysis_batch_id=resolved_analysis_batch_id,
        sales_orders_rows=normalized_sales_orders,
        sales_order_items_rows=normalized_sales_order_items,
        inventory_current_rows=normalized_inventory_current,
        inventory_daily_snapshot_rows=normalized_inventory_daily_snapshot,
    )
