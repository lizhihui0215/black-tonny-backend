...
DAILY_PAYMENT_SNAPSHOT_SOURCE_ENDPOINT = "yeusoft.report.daily_payment_snapshot"


def _latest_daily_payment_snapshot_evidence_path() -> Path:
    analysis_dir = PROJECT_ROOT / "tmp" / "capture-samples" / "analysis"
    candidates = sorted(analysis_dir.glob("daily-payment-snapshot-evidence-chain-*.json"))
    if not candidates:
        raise FileNotFoundError("未找到 daily-payment-snapshot-evidence-chain 分析文件")
    return candidates[-1]


def main() -> int:
    parser = argparse.ArgumentParser(description="把 Yeusoft 每日流水单按 snapshot capture 路线写入 capture。")
    ...
    parser.add_argument(
        "--output",
        help="输出 admission JSON；默认写入 tmp/capture-samples/analysis/daily-payment-snapshot-capture-admission-<timestamp>.json",
    )
    ...
    output_path = (
        ...
        / "capture-samples"
        / "analysis"
        / f"daily-payment-snapshot-capture-admission-{now_local().strftime('%Y%m%d-%H%M%S')}.json"
    )

    evidence_path = _latest_daily_payment_snapshot_evidence_path()
    daily_payment_snapshot_evidence = json.loads(evidence_path.read_text("utf-8"))
    ...
    if not bool(snapshot_detail.get("capture_admission_ready")):
        raise RuntimeError("每日流水单当前尚未满足 snapshot capture 条件，请先刷新 evidence")

    auth = read_login_auth(Path(args.readme))
    access_token, _ = refresh_session_access_token(auth)
    spec = find_report_spec_by_title(REPORT_DOC_PATH, "每日流水单")
    if spec is None or not isinstance(spec.payload, dict):
        raise RuntimeError("未能在 report_api_samples.md 中找到“每日流水单”样本")
    ...
    if status != 200:
        raise RuntimeError(f"每日流水单 baseline 返回 HTTP {status}")

    capture_batch_id = None
    if not args.skip_db:
        init_databases()
        capture_batch_id = create_capture_batch(
            source_name="daily-payment-snapshot-capture-admission",
        ...
        bundle = persist_daily_payment_snapshot_capture_admission_bundle(
            ...
            source_endpoint=DAILY_PAYMENT_SNAPSHOT_SOURCE_ENDPOINT,
            account_context={
    ...
    output = {
        ...
        "capture_batch_id": capture_batch_id,
        "daily_payment_snapshot_evidence_source": str(evidence_path.relative_to(PROJECT_ROOT)),
        "source_endpoint": DAILY_PAYMENT_SNAPSHOT_SOURCE_ENDPOINT,
