...
PRODUCT_SALES_SNAPSHOT_SOURCE_ENDPOINT = "yeusoft.report.product_sales_snapshot"


def _latest_product_sales_snapshot_evidence_path() -> Path:
    analysis_dir = PROJECT_ROOT / "tmp" / "capture-samples" / "analysis"
    candidates = sorted(analysis_dir.glob("product-sales-snapshot-evidence-chain-*.json"))
    if not candidates:
        raise FileNotFoundError("未找到 product-sales-snapshot-evidence-chain 分析文件")
    return candidates[-1]


def main() -> int:
    parser = argparse.ArgumentParser(description="把 Yeusoft 商品销售情况按 snapshot capture 路线写入 capture。")
    ...
    parser.add_argument(
        "--output",
        help="输出 admission JSON；默认写入 tmp/capture-samples/analysis/product-sales-snapshot-capture-admission-<timestamp>.json",
    )
    ...
    output_path = (
        ...
        / "capture-samples"
        / "analysis"
        / f"product-sales-snapshot-capture-admission-{now_local().strftime('%Y%m%d-%H%M%S')}.json"
    )

    evidence_path = _latest_product_sales_snapshot_evidence_path()
    product_sales_snapshot_evidence = json.loads(evidence_path.read_text("utf-8"))
    ...
    if not bool(snapshot_detail.get("capture_admission_ready")):
        raise RuntimeError("商品销售情况当前尚未满足 snapshot capture 条件，请先刷新 evidence")

    auth = read_login_auth(Path(args.readme))
    access_token, _ = refresh_session_access_token(auth)
    spec = find_report_spec_by_title(REPORT_DOC_PATH, "商品销售情况")
    if spec is None or not isinstance(spec.payload, dict):
        raise RuntimeError("未能在 report_api_samples.md 中找到“商品销售情况”样本")
    ...
    if status != 200:
        raise RuntimeError(f"商品销售情况 baseline 返回 HTTP {status}")

    capture_batch_id = None
    if not args.skip_db:
        init_databases()
        capture_batch_id = create_capture_batch(
            source_name="product-sales-snapshot-capture-admission",
        ...
        bundle = persist_product_sales_snapshot_capture_admission_bundle(
            ...
            source_endpoint=PRODUCT_SALES_SNAPSHOT_SOURCE_ENDPOINT,
            account_context={
    ...
    output = {
        ...
        "capture_batch_id": capture_batch_id,
        "product_sales_snapshot_evidence_source": str(evidence_path.relative_to(PROJECT_ROOT)),
        "source_endpoint": PRODUCT_SALES_SNAPSHOT_SOURCE_ENDPOINT,
