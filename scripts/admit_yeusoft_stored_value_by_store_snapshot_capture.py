...
STORED_VALUE_BY_STORE_SNAPSHOT_SOURCE_ENDPOINT = "yeusoft.report.stored_value_by_store"


def _latest_evidence_path() -> Path:
    analysis_dir = PROJECT_ROOT / "tmp" / "capture-samples" / "analysis"
    candidates = sorted(analysis_dir.glob("stored-value-by-store-snapshot-evidence-chain-*.json"))
    if not candidates:
        raise FileNotFoundError("未找到 stored-value-by-store-snapshot-evidence-chain 分析文件")
    return candidates[-1]


def main() -> int:
    parser = argparse.ArgumentParser(description="把 Yeusoft 储值按店汇总按 snapshot capture 路线写入 capture。")
    ...
    parser.add_argument(
        "--output",
        help="输出 admission JSON；默认写入 tmp/capture-samples/analysis/stored-value-by-store-snapshot-capture-admission-<timestamp>.json",
    )
    ...
    output_path = (
        ...
        / "capture-samples"
        / "analysis"
        / f"stored-value-by-store-snapshot-capture-admission-{now_local().strftime('%Y%m%d-%H%M%S')}.json"
    )

    evidence_path = _latest_evidence_path()
    evidence = json.loads(evidence_path.read_text("utf-8"))
    snapshot_detail = dict((evidence.get("stored_value_by_store_snapshot") or {}))
    if not bool(snapshot_detail.get("capture_admission_ready")):
        raise RuntimeError("储值按店汇总当前尚未满足 snapshot capture 条件，请先刷新 evidence")

    auth = read_login_auth(Path(args.readme))
    access_token, _ = refresh_session_access_token(auth)
    spec = find_report_spec_by_title(REPORT_DOC_PATH, "储值按店汇总")
    if spec is None or not isinstance(spec.payload, dict):
        raise RuntimeError("未能在 report_api_samples.md 中找到“储值按店汇总”样本")
    ...
    if status != 200:
        raise RuntimeError(f"储值按店汇总 baseline 返回 HTTP {status}")

    capture_batch_id = None
    if not args.skip_db:
        init_databases()
        capture_batch_id = create_capture_batch(
            source_name="stored-value-by-store-snapshot-capture-admission",
        ...
        bundle = persist_stored_value_by_store_snapshot_capture_admission_bundle(
            ...
            stored_value_by_store_snapshot_evidence=evidence,
            baseline_payload=baseline_payload,
            baseline_request_payload=request_payload,
            source_endpoint=STORED_VALUE_BY_STORE_SNAPSHOT_SOURCE_ENDPOINT,
            account_context={
    ...
    output = {
        ...
        "capture_batch_id": capture_batch_id,
        "stored_value_by_store_snapshot_evidence_source": str(evidence_path.relative_to(PROJECT_ROOT)),
        "source_endpoint": STORED_VALUE_BY_STORE_SNAPSHOT_SOURCE_ENDPOINT,
