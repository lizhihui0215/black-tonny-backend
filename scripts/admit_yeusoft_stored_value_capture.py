...
STORED_VALUE_SOURCE_ENDPOINT = "yeusoft.report.stored_value_card_detail"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="把 Yeusoft 储值卡明细按正式主链候选路线准入 capture。")
    ...
    parser.add_argument(
        "--output",
        help="输出 admission JSON；默认写入 tmp/capture-samples/analysis/stored-value-capture-admission-<timestamp>.json",
    )
    ...
    output_path = (
        ...
        / "capture-samples"
        / "analysis"
        / f"stored-value-capture-admission-{now_local().strftime('%Y%m%d-%H%M%S')}.json"
    )
    ...
    if not bool(stored_value_detail.get("capture_admission_ready")):
        raise RuntimeError("储值卡明细当前尚未满足 capture admit 条件，请先刷新 stored value evidence")

    auth = read_login_auth(Path(args.readme))
    access_token, _ = refresh_session_access_token(auth)
    spec = find_report_spec_by_title(REPORT_DOC_PATH, "储值卡明细")
    if spec is None:
        raise RuntimeError("未能在 report_api_samples.md 中找到“储值卡明细”样本")
    ...
    if not args.skip_db:
        ...
        capture_batch_id = create_capture_batch(
            source_name="stored-value-capture-admission",
        ...
        bundle = persist_stored_value_capture_admission_bundle(
            ...
            source_endpoint=STORED_VALUE_SOURCE_ENDPOINT,
            account_context={
