...
def main() -> int:
    parser = argparse.ArgumentParser(description="把 Yeusoft 库存明细统计按 stockflag 主链候选路线准入 capture。")
    ...
    parser.add_argument(
        "--output",
        help="输出 admission JSON；默认写入 tmp/capture-samples/analysis/inventory-stock-capture-admission-<timestamp>.json",
    )
    ...
    output_path = (
        ...
        / "capture-samples"
        / "analysis"
        / f"inventory-stock-capture-admission-{now_local().strftime('%Y%m%d-%H%M%S')}.json"
    )
    ...
    detail_bundle = admission_bundle["inventory_detail"]
    ...
    report_spec = find_report_spec_by_title(report_doc, "库存明细统计")
    if report_spec is None or not isinstance(report_spec.payload, dict):
        raise RuntimeError("未在 report_api_samples.md 中找到可用的 库存明细统计 payload")

    request_payloads: dict[str, dict[str, object]] = {}
    response_payloads: dict[str, dict[str, object] | list[object]] = {}
    url = maybe_override_url(_make_spec(report_spec.url), api_base_url)
    headers = build_report_auth_headers(url, access_token)
    for stockflag_value in detail_bundle["recommended_stockflag_values"]:
        request_payload = copy.deepcopy(report_spec.payload)
        request_payload["stockflag"] = stockflag_value
        request_payload["page"] = 0
        request_payloads[str(stockflag_value)] = request_payload
        status, payload = perform_request(args.transport, url, request_payload, headers)
        if status != 200:
            raise RuntimeError(f"库存明细统计 stockflag={stockflag_value} 返回 HTTP {status}")
        response_payloads[str(stockflag_value)] = payload

    capture_batch_id = None
    if not args.skip_db:
        init_databases()
        capture_batch_id = create_capture_batch(
            source_name="inventory-stock-capture-admission",
            capture_batch_id=args.capture_batch_id,
        )
        bootstrap_capture_batch(capture_batch_id, auth, auth.company_code or "", api_base_url)
        result_bundle = persist_inventory_detail_capture_admission_bundle(
            capture_batch_id=capture_batch_id,
            inventory_evidence=inventory_evidence,
            stockflag_payloads=response_payloads,
            stockflag_request_payloads=request_payloads,
