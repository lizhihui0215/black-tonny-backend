...
def main() -> int:
    parser = argparse.ArgumentParser(description="把 Yeusoft 收货确认主列表按正式主链候选路线准入 capture。")
    ...
    parser.add_argument(
        "--output",
        help="输出 admission JSON；默认写入 tmp/capture-samples/analysis/receipt-confirmation-capture-admission-<timestamp>.json",
    )
    ...
    output_path = (
        ...
        / "capture-samples"
        / "analysis"
        / f"receipt-confirmation-capture-admission-{now_local().strftime('%Y%m%d-%H%M%S')}.json"
    )
    ...
    if not bool(detail.get("capture_admission_ready")):
        raise RuntimeError("收货确认当前尚未满足 capture admit 条件，请先刷新 receipt confirmation evidence")
    ...
    status, baseline_payload = perform_request(args.transport, RECEIPT_CONFIRMATION_URL, baseline_request_payload, headers)
    if status != 200:
