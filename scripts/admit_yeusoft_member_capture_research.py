...
def main() -> int:
    parser = argparse.ArgumentParser(description="把 Yeusoft 会员中心按 research-capture 路线写入 capture。")
    ...
    parser.add_argument(
        "--output",
        help="输出 research JSON；默认写入 tmp/capture-samples/analysis/member-capture-research-<timestamp>.json",
    )
    ...
    output_path = (
        ...
        / "capture-samples"
        / "analysis"
        / f"member-capture-research-{now_local().strftime('%Y%m%d-%H%M%S')}.json"
    )
    ...
    report_spec = find_report_spec_by_title(report_doc, "会员中心")
    if report_spec is None or not isinstance(report_spec.payload, dict):
        raise RuntimeError("未在 report_api_samples.md 中找到可用的 会员中心 payload")
    ...
    status, baseline_payload = perform_request(args.transport, url, baseline_request_payload, headers)
    if status != 200:
    ...
    if not args.skip_db:
        ...
        capture_batch_id = create_capture_batch(
            source_name="member-capture-research",
