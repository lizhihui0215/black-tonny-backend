...
def main() -> int:
    parser = argparse.ArgumentParser(description="把 Yeusoft 门店盘点单主列表按正式主链候选路线准入 capture。")
    ...
    parser.add_argument(
        "--output",
        help="输出 admission JSON；默认写入 tmp/capture-samples/analysis/store-stocktaking-capture-admission-<timestamp>.json",
    )
    ...
    output_path = (
        ...
        / "capture-samples"
        / "analysis"
        / f"store-stocktaking-capture-admission-{now_local().strftime('%Y%m%d-%H%M%S')}.json"
    )
    ...
    if not bool(detail.get("capture_admission_ready")):
        raise RuntimeError("门店盘点单当前尚未满足 capture admit 条件，请先刷新 store stocktaking evidence")
    ...
    if not baseline_request_payload:
        raise RuntimeError("门店盘点单 evidence 缺少 baseline_payload，暂不执行 capture admit")
