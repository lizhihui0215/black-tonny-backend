...
MEMBER_SALES_RANK_SNAPSHOT_SOURCE_ENDPOINT = "yeusoft.report.member_sales_rank_snapshot"


def _latest_member_sales_rank_snapshot_evidence_path() -> Path:
    analysis_dir = PROJECT_ROOT / "tmp" / "capture-samples" / "analysis"
    candidates = sorted(analysis_dir.glob("member-sales-rank-snapshot-evidence-chain-*.json"))
    if not candidates:
        raise FileNotFoundError("未找到 member-sales-rank-snapshot-evidence-chain 分析文件")
    return candidates[-1]


def main() -> int:
    parser = argparse.ArgumentParser(description="把 Yeusoft 会员消费排行按 snapshot capture 路线写入 capture。")
    ...
    parser.add_argument(
        "--output",
        help="输出 admission JSON；默认写入 tmp/capture-samples/analysis/member-sales-rank-snapshot-capture-admission-<timestamp>.json",
    )
    ...
    output_path = (
        ...
        / "capture-samples"
        / "analysis"
        / f"member-sales-rank-snapshot-capture-admission-{now_local().strftime('%Y%m%d-%H%M%S')}.json"
    )

    evidence_path = _latest_member_sales_rank_snapshot_evidence_path()
    member_sales_rank_snapshot_evidence = json.loads(evidence_path.read_text("utf-8"))
    ...
    if not bool(snapshot_detail.get("capture_admission_ready")):
        raise RuntimeError("会员消费排行当前尚未满足 snapshot capture 条件，请先刷新 evidence")

    auth = read_login_auth(Path(args.readme))
    access_token, _ = refresh_session_access_token(auth)
    spec = find_report_spec_by_title(REPORT_DOC_PATH, "会员消费排行")
    if spec is None or not isinstance(spec.payload, dict):
        raise RuntimeError("未能在 report_api_samples.md 中找到“会员消费排行”样本")
    ...
    if status != 200:
        raise RuntimeError(f"会员消费排行 baseline 返回 HTTP {status}")

    capture_batch_id = None
    if not args.skip_db:
        init_databases()
        capture_batch_id = create_capture_batch(
            source_name="member-sales-rank-snapshot-capture-admission",
        ...
        bundle = persist_member_sales_rank_snapshot_capture_admission_bundle(
            ...
            source_endpoint=MEMBER_SALES_RANK_SNAPSHOT_SOURCE_ENDPOINT,
            account_context={
    ...
    output = {
        ...
        "capture_batch_id": capture_batch_id,
        "member_sales_rank_snapshot_evidence_source": str(evidence_path.relative_to(PROJECT_ROOT)),
        "source_endpoint": MEMBER_SALES_RANK_SNAPSHOT_SOURCE_ENDPOINT,
